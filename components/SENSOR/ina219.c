                                        ...
                                        void (*valve_close_func)(void))
{
    ...
    while (1) {
         ...
         * 不能再等稳定后才密集); dense 段(稳定后)按 2ms 抽样省容量; 大变化点, 心跳保底 */
        bool record = (rec_cnt == 0)
                   ...
                   || (fabsf(i - i_prev) >= CHANGE_RECORD_A)
    ...
    if (i_max < NO_PEAK_ABORT_A) {
        ...
        if (r_ratio >= RIPPLE_MIN_AMP_RATIO) {
            ...
            /* 上升沿检测(迟滞过中点 + 去抖 ≥2ms)
             * 迟滞 = 15% 纹波幅度: 滤掉上升沿处的阶梯/过冲伪沿(如 0→30mA→56mA 阶梯),
             * 避免把真实周期(如 120ms)切成碎段导致周期一致性检查失败 */
            float hyst = r_amp * 0.15f;
